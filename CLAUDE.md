...
**Analytics**: `NEXT_PUBLIC_GA_MEASUREMENT_ID`, `NEXT_PUBLIC_META_PIXEL_ID`, `NEXT_PUBLIC_CONTENTSQUARE_TAG_ID`, `NEXT_PUBLIC_SITE_URL` (default: `https://studiotak.co`). Google Analytics and Contentsquare are loaded only after the visitor opts into Analytics; Meta Pixel is loaded only after they opt into Marketing.
...
| `contact` | Two-column: Resend contact form on the left, square media on the right. Includes email validation, Turnstile protection, and a success state. | Lead capture, contact forms, inquiries | `heading`, `body`, `anchor`, `media` |
