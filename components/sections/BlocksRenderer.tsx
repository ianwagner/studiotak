...
const StepsBlockSection = ({
  ...
  const resolvedSteps = useMemo<FeatureItem[]>(() => {
    const cleaned = (block.items ?? []).filter((item) => (item.title?.trim() || item.body?.trim()));
    if (cleaned.length) return cleaned;
    return [
      { title: "Add your steps", body: "Pick components from the library to outline each milestone." },
      { title: "Tune the order", body: "Reorder or trim steps to match this page." },
      { title: "Preview the flow", body: "Use the controls to step through the timeline with motion." }
    ];
  }, [block.items]);
  const [activeIdx, setActiveIdx] = useState(0);
  const stepCount = resolvedSteps.length || 1;
  const progressPct = stepCount > 1 ? (activeIdx / (stepCount - 1)) * 100 : 0;
  const activeStep = resolvedSteps[activeIdx] ?? resolvedSteps[0];
  const badge = activeStep?.badge ?? activeStep?.type ?? activeStep?.industry;
  const minHeightMultiplier = Math.max(stepCount, 2);
  const headingText = block.heading?.trim() || "Steps";
  const kicker = block.body?.trim() || undefined;

  useEffect(() => {
    setActiveIdx((prev) => clampNumber(prev, 0, Math.max(stepCount - 1, 0)));
  }, [stepCount]);

  const handleSelect = useCallback(
    (next: number) => setActiveIdx(clampNumber(next, 0, Math.max(stepCount - 1, 0))),
    [stepCount]
  );
  const handleNext = useCallback(() => handleSelect(activeIdx + 1), [activeIdx, handleSelect]);
  const handlePrev = useCallback(() => handleSelect(activeIdx - 1), [activeIdx, handleSelect]);
  ...
  useMotionValueEvent(scrollYProgress, "change", (value) => {
    ...
    const next = Math.round(value * (stepCount - 1));
    setActiveIdx((prev) => (prev === next ? prev : clampNumber(next, 0, stepCount - 1)));
  });
  ...
  return (
    <AnimatedSection
      ...
      index={index}
      variant="plain"
      style={{ width: "100%", maxWidth: "min(var(--max-width), 1200px)", margin: "0 auto" }}
    >
      <style>{`
        [data-steps-shell] {
          background: linear-gradient(145deg, rgba(255, 112, 11, 0.08), rgba(255, 255, 255, 0.02));
          border: 1px solid var(--border-strong);
        ...
        [data-steps-track]::-webkit-scrollbar {
          height: 6px;
        }
        [data-steps-track]::-webkit-scrollbar-thumb {
        ...
        [data-steps-node][data-active="true"] .dot {
          border-color: var(--accent);
          background: var(--accent);
          color: #fff;
          transform: scale(1.05);
        }
        [data-steps-node] .label {
          font-size: 13px;
          color: var(--muted);
          white-space: nowrap;
        }
        ...
        [data-steps-nav] button:disabled {
          opacity: 0.45;
          cursor: not-allowed;
        }
      ...
      <div
        ...
        style={{
          position: "relative",
          minHeight: `calc(${minHeightMultiplier} * 100vh)`,
          width: "100%",
      ...
      >
        <div
          ...
          style={{
            position: "sticky",
            top: Math.max(headerHeight, 64),
            minHeight: "calc(100vh - 30px)",
            padding: 18,
            display: "grid",
            gap: 18,
            alignContent: "flex-start",
            background: "linear-gradient(135deg, rgba(255,112,11,0.08), rgba(255,255,255,0.02))"
          }}
        >
          <div style={{ position: "relative" }}>
            ...
            <div
              data-steps-progress
              style={{ width: stepCount === 1 ? 28 : `min(100%, calc(${progressPct}% + 14px))` }}
              aria-hidden
            />
            <div
              data-steps-track
              style={{
                gridTemplateColumns: `repeat(${stepCount}, minmax(140px, 1fr))`
              }}
            >
              {resolvedSteps.map((step, stepIdx) => (
                <button
                  key={`${step.title}-${stepIdx}`}
                  type="button"
                  data-steps-node
                  data-active={stepIdx === activeIdx}
                  onClick={() => handleSelect(stepIdx)}
                  aria-pressed={stepIdx === activeIdx}
                >
                  <span className="dot">{stepIdx + 1}</span>
          ...
          <div style={{ textAlign: "center" }}>
            <SectionHeading eyebrow={block.eyebrow} title={headingText} kicker={kicker} align="center" />
          </div>
          <div style={{ display: "grid", justifyItems: "center" }}>
            <AnimatePresence mode="wait">
              <motion.div
                key={`${activeStep?.title ?? "step"}-${activeIdx}`}
              ...
              >
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", justifyContent: "center" }}>
                    <Pill>Step {activeIdx + 1}</Pill>
                    {badge ? <span className="tag">{badge}</span> : null}
                  </div>
                  {activeStep?.icon?.url ? (
                    <img
                      src={activeStep.icon.url}
                      alt={activeStep.icon.alt ?? ""}
                      style={{
                        width: 72,
                        height: 72,
                        borderRadius: 14,
                        border: "1px solid var(--border-strong)",
                        objectFit: activeStep.mediaFit === "contain" ? "contain" : "cover",
                        background: "rgba(255,255,255,0.04)",
                        margin: "0 auto"
                      }}
                    />
                  ) : null}
                  <h3 style={{ margin: 0, fontSize: 26, lineHeight: 1.25, textAlign: "center" }}>{activeStep?.title ?? "Step"}</h3>
                  {activeStep?.body ? (
                    <p style={{ margin: 0, color: "var(--muted)", fontSize: 15, lineHeight: 1.6, textAlign: "center" }}>
                      {activeStep.body}
                    </p>
                  ) : null}
                  {activeStep?.href ? (
                    <AnchorAwareLink className="nav-link" href={activeStep.href} style={{ justifySelf: "center" }}>
                      Learn more
                    </AnchorAwareLink>
                  ) : null}
                </div>
          ...
          <div data-steps-nav>
            <button type="button" onClick={handlePrev} disabled={activeIdx === 0} aria-label="Previous step">
              <ChevronLeftIcon />
            </button>
            <button type="button" onClick={handleNext} disabled={activeIdx >= stepCount - 1} aria-label="Next step">
              <ChevronRightIcon />
            </button>
          </div>
        </div>
...
const ShowcaseBlockSection = ({
  ...
  const forceScroll = viewportWidth !== null && viewportWidth < 1250;
  ...
  const overlapBase = isNarrow ? 54 : clampNumber(Math.round(availableWidth / 18), 42, 120);
  const overlapBoost =
    viewportWidth === null
      ? 1
      : viewportWidth < 900
      ...
      ? 1.4
      : 1;
  const overlap = clampNumber(Math.round(overlapBase * overlapBoost), 42, 210);
          ...
          >
            {displayItems.length ? (
              displayItems.map((item, cardIdx) => {
                ...
                const baseTranslateY = cardIdx % 2 === 0 ? -4 : 6;
                const hoverTranslateY = cardIdx % 2 === 0 ? -2 : 4;
                ...
                return (
                  <motion.div
                    ...
                    style={{
                      ...
                      translateY: isHovered ? hoverTranslateY : baseTranslateY,
      ...
      <div className="grid" style={{ gap: 24 }}>
        {blocks.map((block, index) => {
          ...
          if (block.type === "hero" || block.type === "thirds") {
            ...
            element = <StepsBlockSection key={key} block={block} index={index} headerHeight={headerHeight} />;
