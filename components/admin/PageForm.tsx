...
const isStepsBlock = (block: EditableBlock): block is StepsBlock => block.type === "steps";
...
const isFeatureItemsBlock = (block: EditableBlock): block is FeaturesBlock | ScrollGalleryBlock | StepsBlock =>
  isFeaturesBlock(block) || isScrollGalleryBlock(block) || isStepsBlock(block);
...
const newStepsBlock = (): StepsBlock => ({
  id: crypto.randomUUID(),
  type: "steps",
  adminLabel: "Steps block",
  anchor: "",
  eyebrow: "Steps",
  heading: "Steps heading",
  body: "Describe the steps and how to move through them.",
  items: [
    { title: "Step 1", body: "First milestone in the flow." },
    { title: "Step 2", body: "Second milestone with a short description." },
    { title: "Step 3", body: "Wrap up with the final step." }
  ],
  enableDarkModeOnScroll: false
});
...
export function PageForm({
  ...
  const handleStepsFieldChange = (
    idx: number,
    field: keyof Omit<StepsBlock, "id" | "type" | "items">,
    value: StepsBlock[keyof Omit<StepsBlock, "id" | "type" | "items">]
  ) => {
    updateBlock(idx, (block) => (isStepsBlock(block) ? { ...block, [field]: value } : block));
  };
  ...
  const addStepItem = (idx: number) => {
    updateBlock(idx, (block) => {
      if (!isStepsBlock(block)) return block;
      const nextItems = [...(block.items ?? []), { title: "New step", body: "Brief description" }];
      return { ...block, items: nextItems };
    });
  };
            ...
            <div className="grid" style={{ gap: 12, padding: 12 }}>
              <div className="grid" style={{ gap: 12, gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))" }}>
                <div className="field-group">
                  ...
                  <input
                    className="input"
                    value={block.eyebrow ?? ""}
                    onChange={(e) => handleStepsFieldChange(idx, "eyebrow", e.target.value)}
                    placeholder="Steps"
                  />
                </div>
                <AnchorField
                  value={block.anchor ?? ""}
                  onChange={(value) => handleStepsFieldChange(idx, "anchor", value)}
                  placeholder={block.anchor || "steps"}
                />
              </div>
              <div className="field-group">
                <label>Heading</label>
                <input
                  className="input"
                  value={block.heading ?? ""}
                  onChange={(e) => handleStepsFieldChange(idx, "heading", e.target.value)}
              ...
              <div className="field-group">
                ...
                <textarea
                  rows={3}
                  value={block.body ?? ""}
                  onChange={(e) => handleStepsFieldChange(idx, "body", e.target.value)}
                  placeholder="Quick summary for this step sequence"
                />
              </div>
              <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <input
                  type="checkbox"
                  checked={!!block.enableDarkModeOnScroll}
                  onChange={(e) => handleStepsFieldChange(idx, "enableDarkModeOnScroll", e.target.checked)}
                />
                <span>Trigger dark mode while this section is in view</span>
              </label>
              <div className="grid" style={{ gap: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                  ...
                  <button type="button" className="btn secondary" onClick={() => addStepItem(idx)}>
                    + Add step
                  </button>
                </div>
                {(block.items ?? []).map((item, itemIdx) => (
